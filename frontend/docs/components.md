...
`@/components/ui/Input`. On mobile / touch, form controls are **16px** (`globals.css`) so iOS does not zoom on focus.
...
- **`/agenda`** — one row per **scheduled occurrence** (event_item). Renders `AgendaRow` (date block · title + location · time range · price · availability label). On **mobile**, the time range sits on a **second line below the date** in the date block and **price** sits under the location line; from **`sm` up**, time and price sit in a **fixed pair** (`w-40` / `w-24`, right-aligned, `tabular-nums`) with extra padding before the status button. Sold-out occurrences stay visible with a **Wachtlijst** status button (opens the waitlist modal for that session; not hidden) and the row is slightly muted (`opacity-70`, gray title/date/time/price). The **entire row** is one link to the product PDP except the status button; hover underlines the title (no text colour change). Additional `date` single-day filter rendered as a `AgendaDayPicker` (month calendar) above the shared sidebar. Future occurrences only. Sort stays chronological (default `start_date`, eerstvolgende eerst) or price; live search does **not** switch to `relevance`. Unfiltered `/agenda` with **`sort=start_date`** or **`sort=start_date_desc`** uses a Next.js hard cache (600 s), same pattern as Ons aanbod. State + serializer in `app/(main)/agenda/_state/url.ts`. Data from `commerceClient.getAgendaPaginated()` → `GET /store/agenda`.
- **VA Thuis (on-demand)** — dark-themed section at `/va-thuis` (CMS landing), `/va-thuis/ons-aanbod` (catalog), `/va-thuis/[…]` (CMS sub-pages or bundle PDP). Components in `@/components/vathuis/*`; `VaThuisCmsPage` renders CMS blocks on dark background. Catalog uses `GET /store/vathuis` via `/api/plp/vathuis`; filters: category + docent only. Ons aanbod / Agenda **Beschikbaarheid** labels `pre_recorded` as **VAthuis** (always listed, with the VAthuis catalog count on Ons aanbod). Checking it filters the current page (`delivery_type=pre_recorded`); Ons aanbod then keeps those products in the grid and shows the same category/docent filters as `/va-thuis/ons-aanbod`. Default Ons aanbod (no delivery filter) still excludes VAthuis. VA Thuis products on `/ons-aanbod/[handle]` redirect to `/va-thuis/[handle]`. They appear in `/zoeken` and QuickSearch. CMS: `page` with `isVaThuis` (slug `va-thuis/…`). See [VATHUIS.md](./VATHUIS.md).
