...
**Productkaarten** — `productRowBlock`: Section title + horizontal **carousel** of up to four **`PlpEventCard`** items (snap scroll; chevron prev/next from `sm` **only when not all cards fit**). **Source** in Sanity: handpicked (4 product refs), automated (`bestsellers` → `GET /store/events?sort=popularity`, `newest` → `sort=newest`), or personalized (client-only: wishlist handles if any, else recently viewed; block hidden when empty). Automated fetch failures (e.g. Medusa unavailable during CI build) render an empty carousel instead of failing the page. Optional footer **`Button` primary** CTA (`ctaLabel` + `ctaUrl`). See Sanity **block layout** doc. Homepage migration: `npm run migrate:homepage-product-row --prefix sanity`.
...
**Shared components** (`@/components/plp/*`) accept `basePath` so they pick the correct URL serializer client-side (`src/lib/filter-url-helpers.ts`: Ons aanbod vs `/agenda`, including the extra `date` field). Reused from both routes: `PlpTabs`, `PlpSearchBar` + `PlpLiveListing` (inline real-time search on Ons aanbod: results update as you type via `/api/plp/events`, URL synced with `history.replaceState`), `PlpFilterSidebar`, `PlpActiveChips` (supports `extraChips` for non-PLP fields like `date`; pass `cityOptions={facets?.cities}` so plaats chips show labels like “Den Haag” instead of slugs), `PlpSortSelect` (accepts custom `options`), `PlpInfiniteResults` / `AgendaInfiniteResults` (infinite scroll + “Laad meer activiteiten” append), `PlpEmptyState`, `PlpHeader`, `PlpBreadcrumbs`, `PlpBanner`.
...
**Shared presentation logic** lives in `src/lib/event-status-presentation.ts`: `presentationForAvailabilityStatus` (agenda CTAs), `classNameForProductBadge` (free-text keyword badge styles), `plpEventLocationLines` (PLP tile location: single city name, **Op locatie** when multiple cities, **Online** for online-only; hybrid products render **Op locatie** and **Online** on separate lines with map-pin and camera icons), `isOnlineOnlyEvent`, `shouldShowEventDates` (hide dates on online-only products and hybrid products whose on-site sessions are all past), `plpEventHasMultipleDates` (prefix card date with **Vanaf** when there are multiple future on-site session dates), and `eventPricePrefixLabel` (**Voor** when all variant prices match, **Vanaf** when they differ; bundle-only products omit the prefix on the booking panel). Extend the keyword lists there so new UI stays consistent.
...
**Delivery type icons** (`DeliveryTypeIcon` in `@/components/ui`): online activities show a **blue camera** icon; on-site activities show a **red map pin**. Used on `AgendaRow` and `PlpEventCard` (one icon per location line).

## Product detail (PDP)

**Image gallery** (`PdpImageGallery`): product images render as a compact row of thumbnails (up to four, deduped from `image_urls`), not a full-width carousel. Tiles fill the container width in a **2×2** grid on mobile and **1×4** from `sm` up, with **3:2** aspect ratio and responsive gaps (`gap-3` → `gap-5`). Optional per-image **caption** shows a white **i** control that opens a tooltip; wire captions when available via `PdpGalleryImage.caption`.

**Sessions** (`PdpLocationTabs`): online variants (`delivery_type: online`) render in one compact block above **Fysieke sessies** (heading, session row, and Zoom/replay info share a light gray background and border). Each row shows date above time, optional instructor, price, and CTA. Offline variants only appear in **Fysieke sessies** (heading editable via `pdp.labels.physicalSessionsHeading`). When more than one physical session exists, a sort dropdown (datum / locatie) appears beside the heading and **Locatie** / **Datum** column headers are clickable (toggle asc/desc). City tabs apply to physical sessions only. On **mobile** (`< md`), each physical session is a stacked card; from **`md` up**, column tables are used (venue as second line under city when set).
