...
- **Category PLP** — `/ons-aanbod/{slug}` (title “Ons aanbod in {label}”), same shared `PlpListingPage` as the base PLP and city pages. `/ons-aanbod/[handle]` resolves **category first**, then **product type** (reis / studiedag / wandeling / workshop), otherwise product PDP. Legacy `?category=` alone redirects to the path URL. Selecting a **second category** (or opening a multi-category query on a category path) navigates to `/ons-aanbod?category=…&category=…` so filter checkboxes stay in sync. **Reset** / **Wis alle filters** on a path-scoped PLP (category, product type, or city) navigates to `/ons-aanbod` and clears the path filter; Agenda reset stays on `/agenda`, VA Thuis catalog reset stays on `/va-thuis/ons-aanbod`.
