...
**Highlighted teacher** (`PdpFeaturedInstructor` in `PdpBookingPanel`): when Salesforce `Highlighted_Teacher__c` is set, the booking panel shows photo, name, title, and bio (`Web_Body__c` as plain text). Same block on regular and VA Thuis PDPs (dark variant on VA Thuis).
