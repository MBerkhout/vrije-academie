...
Two tabbed listing routes share the same filter model and components under `@/components/plp`. Salesforce **Zichtbaar op Website** unchecked groups are not listed (Medusa `draft`; PDP 404s). Hidden child products (including **Externe verhuur**) are omitted as sessions; the group can still appear without upcoming dates.
