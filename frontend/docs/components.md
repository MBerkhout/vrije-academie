...
- **Desktop**: utility links row, yellow divider, main links + pill **Zoek** control (search icon + label). Opening it renders **QuickSearch** — a full-window overlay with live grouped suggestions from Medusa OpenSearch (`GET /store/search?mode=suggest`): **Categorieën** first, then **Producten** (only products with a future session, sold-out included), Plaatsen, Pagina's; recent searches (localStorage); **Vaak gezocht** from Sanity. Submit navigates to `/zoeken?q=…`. Escape or close (X) dismisses. Active route and nav link hovers use **`va-yellow`** underline.
- **Mobile**: Sticky at the top while scrolling (`position: sticky`). Logo monogram + **Vrije Academie** wordmark (`logo_text.svg`) in the branding row. Zoeken / Mand / Menu actions; **QuickSearch** is full-screen. Yellow **secondary nav** (33% each): **Ons aanbod**, **VAthuis**, **Login** (→ **Mijn account** when logged in); hamburger opens a two-column drawer.
...
**Rich text**: Inline **inlineButton** marks in `PortableText` (Text block, form intros, etc.) use the same `Button` with **`size="md"`** and **`mx-1`** in running text, so **primary** and **secondary** match stand-alone CTAs; **text link** maps to `ghost`. Bullet and numbered lists use **`list-outside`** with left padding so wrapped lines stay in a column to the right of the marker (not under it).
...
`@/components/ui/Input`. On mobile / touch, form controls are **16px** (`globals.css`) so iOS does not zoom on focus. When `type="password"`, an eye button toggles visibility (`Toon wachtwoord` / `Verberg wachtwoord`). The same control is built into `ValidatedInput` (login, checkout, register).
...
**Hero block** and **Banner slider** share **`ImageSlideStage`**: multi-slide **yellow chevron** previous/next at the **bottom left and right**, **dot** indicators centered along the bottom, arrows above slide links, autoplay from Sanity **Autoplay** / **Autoplay Interval**. Hero slide titles use **`h1`**; banner slide titles use **`h2`**. Hero: 2/3 column slider (`rounded-lg`, variable height with right column) plus top panel and **Meld je aan** (`newsletterEnabled`; unset or true = shown). Banner: fixed **`h-[400px]`**, **`w-full`**, no radius; layout defaults to **full** width in Sanity. Promotion analytics: `homepage_jumbotron_*` (hero) vs `banner_slider_*` (banner). **LCP** (hero only): first slide background via `SanityImage` with `priority`, passed from `BlockRenderer` as a server slot; banner slides stay lazy. Other CMS images remain lazy by default. `SanityImage` in **`fill`** / **`contain`** mode uses `w` + `fit=max`; default quality **80** (`next.config.js` `[75, 80, 90]`).
...
**Categories block**: **Title** (with **`4px`** **`va-yellow`** bar, **`max-width: 100px`**) sits **above** the full-width category grid. **Intro text** from Sanity is not rendered. When enabled, the **CTA** is **right-aligned** below the grid as an **underlined** text-style control (`Button` `ghost`, **uppercase**); default underline matches text (**`va-black`**), **`va-yellow`** underline **on hover**; the Sanity **CTA label** is shown followed by a **` >`** (default *Bekijk ons volledige aanbod* for new documents). Grid: **2** columns below **`md`**, **3** from **`md`** to **`lg`**, then **4** or **8** (Sanity **columns desktop**) from **`lg`**. Tiles are horizontal cards (category title with **“Bekijk”** chevron **below** it, **yellow** divider, **75px**-wide image strip, **`min-height: 90px`**), square corners, grid `gap-2` / `md:gap-3`, hover shadow, focus ring. Tile images use `SanityImage` `fill` with **quality 90**; the Sanity URL uses `fit=max` (no 16:9 crop) so portrait sources stay sharp in the strip. Prefer category images at least ~400px on the short edge.
...
Two tabbed listing routes share the same filter model and components under `@/components/plp`. Salesforce **Zichtbaar op Website** unchecked groups are not listed (Medusa `draft`; PDP 404s). **Externe verhuur** groups are also drafted and omitted from Agenda/Ons aanbod (title or record type contains `verhuur`), including leftover rows in a stale listing snapshot. Hidden child products (including **Externe verhuur** sessions on a visible group) are omitted as sessions; groups with no remaining public future sessions are hidden from Ons aanbod.

- **Product listing (PLP)** — path is `PLP_BASE_PATH` in `src/lib/routes.ts` (default `/ons-aanbod`; must match the `app/(main)/ons-aanbod` route segment). One tile per **product** (course/lecture); `PlpEventCard` grid (**2** columns from the smallest breakpoint, **3** from `lg`; **`gap-2`** below `md`, then **`gap-4`** / **`gap-6`** at larger breakpoints). On mobile, card titles use **`text-sm`** with up to **3** lines; CTA reads **Bekijk →** (desktop: **Bekijk meer →**). When a date is shown, it is always prefixed with **Vanaf**; the price is always **Vanaf €…** (not **Voor**). Sort options include relevance, newest, price, and earliest start date (`?sort=start_date`). Unfiltered `/ons-aanbod` with **`sort=order`** or **`sort=start_date`** uses a Next.js hard cache (600 s); if SSR Medusa fetch fails, `PlpLiveListing` retries via `/api/plp/events`. **Wishlist** — heart button (top-right on the image) toggles saved courses via `PlpEventCardWishlistButton` (`useWishlist`: `localStorage` for guests, merged into `customer.metadata.va_wishlist` on login). Filter sidebar group **Soort activiteit** uses `product_type[]` on `GET /store/events`. Filter, chips, and dedicated landing pages use the **plural** label from **General Settings → Product listing page → Product type plurals** (fallback **Reizen** for `reis`; PDP badges keep the singular Salesforce name). Dedicated landing pages: `/ons-aanbod/reis`, `/ons-aanbod/studiedag`, `/ons-aanbod/wandeling`, `/ons-aanbod/workshop` (title “Ons aanbod in {plural}”; category slug wins on collision). State + serializer in `app/(main)/ons-aanbod/_state/url.ts`. Data from `commerceClient.getEventsPaginated()` → `GET /store/events`. Products without at least one **future session** (`start_at >= now` or on-demand) are excluded from the listing; **sold-out future sessions stay listed** (fully sold-out cards show **Wachtlijst**, opening the waitlist modal). **Linked-online slave** catalogs (separate `ONLINE - …` Salesforce groups) never appear on Ons aanbod. City, day-part, and period filters/facets count **future offline sessions only**. **Agenda** (`/agenda`, `GET /store/agenda`) uses the same product-level visibility rules. Admin **Show on Ons aanbod** (`show_in_plp`) is stored but not enforced on the storefront yet; the digitale cadeaubon handle is always excluded server-side.
- **Category PLP** — `/ons-aanbod/{slug}` (title “Ons aanbod in {label}”), same shared `PlpListingPage` as the base PLP and city pages. `/ons-aanbod/[handle]` resolves **category first**, then **product type** (reis / studiedag / wandeling / rondleiding / workshop), otherwise product PDP. Legacy `?category=` alone redirects to the path URL. Selecting a **second category** (or opening a multi-category query on a category path) navigates to `/ons-aanbod?category=…&category=…` so filter checkboxes stay in sync. **Reset** / **Wis alle filters** on a path-scoped PLP (category, product type, or city) navigates to `/ons-aanbod` and clears the path filter; Agenda reset stays on `/agenda`, VA Thuis catalog reset stays on `/va-thuis/ons-aanbod`.
...
- **`/agenda`** — one row per **scheduled occurrence** (event_item) where **start and end fall on the same calendar day** (Europe/Amsterdam); multi-day collegereeks/reizen are omitted. **VA-thuis** occurrences (`record_type: vathuis` or `delivery_type: pre_recorded`) are excluded. Renders `AgendaRow`. On **mobile** (`< sm`) the card stacks: **date + weekday + time** (price right-aligned on that row) → **title** → **location** → full-width status button. From **`sm` up**, the row is date block · title + location · time range · price · availability label. Below **`xl`**, time sits **above** price (no fixed pair widths) so the title column stays wide; from **`xl`**, time and price sit in a **fixed pair** (`w-40` / `w-24`, right-aligned, `tabular-nums`) with extra padding before the status button. The **title wraps on word boundaries** (no line-clamp / ellipsis, no mid-word breaks). Sold-out occurrences stay visible with a **Wachtlijst** status button (opens the waitlist modal for that session; not hidden) and the row is slightly muted (`opacity-70`, gray title/date/time/price). The **entire row** is one link to the product PDP except the status button; hover underlines the title (no text colour change). Additional `date` single-day filter rendered as a `AgendaDayPicker` (month calendar) above the shared desktop sidebar, and as the first **Agenda** group (open by default) in the mobile filter drawer. Future occurrences only. Sort stays chronological (default `start_date`, eerstvolgende eerst) or price; live search does **not** switch to `relevance`. Unfiltered `/agenda` with **`sort=start_date`** or **`sort=start_date_desc`** uses a Next.js hard cache (600 s), same pattern as Ons aanbod. State + serializer in `app/(main)/agenda/_state/url.ts`. Data from `commerceClient.getAgendaPaginated()` → `GET /store/agenda`.
- **VAthuis (on-demand)** — public name **VAthuis** (no space); dark-themed section at `/va-thuis` (CMS landing), `/va-thuis/ons-aanbod` (catalog), `/va-thuis/[…]` (CMS sub-pages or bundle PDP). Components in `@/components/vathuis/*`; `VaThuisCmsPage` renders CMS blocks on dark background. Catalog uses `GET /store/vathuis` via `/api/plp/vathuis`; filters: category + docent only. Ons aanbod **Beschikbaarheid** labels `pre_recorded` as **VAthuis** (always listed, with the VAthuis catalog count). Checking it filters Ons aanbod (`delivery_type=pre_recorded`) and keeps those products in the grid with the same category/docent filters as `/va-thuis/ons-aanbod`. Default Ons aanbod (no delivery filter) still excludes VAthuis. Agenda omits the VAthuis checkbox and ignores leftover `delivery_type=pre_recorded` query params. VAthuis products on `/ons-aanbod/[handle]` redirect to `/va-thuis/[handle]`. They appear in `/zoeken` and QuickSearch. CMS: `page` with `isVaThuis` (slug `va-thuis/…`). See [VATHUIS.md](./VATHUIS.md).
...
**Shared components** (`@/components/plp/*`) accept `basePath` so they pick the correct URL serializer client-side (`src/lib/filter-url-helpers.ts`: Ons aanbod vs `/agenda`, including the extra `date` field). Reused from both routes: `PlpTabs`, `PlpSearchBar` + `PlpLiveListing` / `AgendaLiveListing` (inline real-time search: results update as you type via `/api/plp/events` or `/api/agenda/items`, URL synced with `history.replaceState`; `?q=` requires every term to match), `PlpFilterSidebar`, `PlpActiveChips` (supports `extraChips` for non-PLP fields like `date`; pass `cityOptions={facets?.cities}` so plaats chips show labels like “Den Haag” instead of slugs), `PlpSortSelect` (accepts custom `options`), `PlpInfiniteResults` / `AgendaInfiniteResults` (infinite scroll + “Laad meer activiteiten” append), `PlpEmptyState`, `PlpHeader`, `PlpBreadcrumbs`, `PlpBanner`.

**Infinite listing**: The first page loads server-side; additional pages fetch via `GET /api/plp/events` or `GET /api/agenda/items` and append below the grid/list. Scrolling near the bottom auto-loads the next batch; the load-more button triggers the same fetch. Legacy `?page=2` URLs redirect to page 1 without the `page` param. Extra loaded pages are kept when the first page is resent, and also across a listing remount after Back (module cache keyed by filters/sort), so infinite scroll does not restart at page 1. Overlapping load-more responses drop already-seen ids. The list only resets when filters or sort change. **Back from a product**: listing cards have a stable HTML id (`product-{handle}`, agenda rows `agenda-{id}-{variant}`). Clicking a card stamps that id on the listing URL hash and stores the current `scrollY`. Returning to the listing restores that offset before paint (`useLayoutEffect`) so the page does not flash at the top, then confirms the card is in view and, if needed, reloads infinite-scroll batches until it exists (`ListingReturnScroll`, `useEnsureListingAnchorLoaded`).
...
**Plaats filter** (`PlpFilterSidebar`): on desktop the group is **expanded by default** (mobile drawer keeps all groups collapsed). Cities are ordered by **most events** (facet count, descending; alphabetical tie-break). Without a search query, the checklist shows **5** options with a bottom fade; **Meer bekijken** (down chevron) sits in that fade overlay—no separate bordered row; same hover as **Meer tonen**. After expanding, all places are listed; if there are **more than 10**, the list scrolls inside a max-height region (~10 rows). Typing in the plaats search shows matching results in the same scroll pattern when needed. All listing filters (including plaats, docent, beschikbaarheid, dagdeel, and periode months) only appear when they still have a **future** activity; plaats + dagdeel/periode must match the same session.

**Periode filter** (`PlpFilterSidebar`): month and voorjaar/najaar shortcuts hide past months and months with no upcoming sessions (`facets.months`). Each month appears once; a month or season is listed when any future year still has sessions (Jan 2027 and Jan 2030 both enable **Jan**). Selecting a month or season matches that calendar period in **every** year; combined with plaats or dagdeel that **same** session must match. The card date is the earliest matching session.

**Mobile filters** (`PlpFilterSidebar` with `mobileOnly`): trigger shows a sliders icon, stronger border, and active-count badge. Opens a **full-screen** panel (`lg` and up keep the desktop sidebar). Each time the panel opens, **Beschikbaarheid**, **Categorie**, **Docent**, and **Plaats** start **expanded** (other groups stay collapsed); categorie and plaats keep their truncated lists with **Meer tonen** / **Meer bekijken** until expanded. On **Agenda**, an **Agenda** day-picker group is prepended and also starts **expanded** (`leadingFilterGroup`); a selected day counts toward the trigger badge. Group titles use **`text-base`** (desktop sidebar keeps **`text-sm`**). When a group has active selections, a yellow count badge appears next to its title (same style as the trigger). Header: title + underlined **Wis alle filters**; scrollable checklist area; fixed footer **Filters sluiten** (yellow, safe-area padding) so content is not covered. Body scroll is locked while open. Used on Ons aanbod and Agenda listing pages.
...
| `almost_full`  | Bijna vol (≤ 30% capaciteit vrij, afgerond naar boven; zonder capaciteit: ≤ 3 plekken) | `#f7373d` / hover `#E0282E` |
| `sold_out`     | Wachtlijst (opens waitlist)  | `bg-va-gray`          |
...
**Shared presentation logic** lives in `src/lib/event-status-presentation.ts`: `presentationForAvailabilityStatus` (agenda CTAs), `classNameForProductBadge` (free-text keyword badge styles), `plpEventDeliveryTypeDisplay` (PLP tile delivery icon next to the title: red map pin for on-site, blue camera for online-only, pin **/** camera when both), `plpEventLocationLines` (legacy text labels for other consumers), `isOnlineOnlyEvent`, `shouldShowEventDates` (hide dates on online-only products and hybrid products whose on-site sessions are all past), `plpEventHasMultipleDates` (multiple future on-site session dates), and `eventPricePrefixLabel` (PDP booking panel: **Voor** when all variant prices match, **Vanaf** when they differ; bundle-only products omit the prefix). Listing cards (`PlpEventCard`) always use **Vanaf** for both date and price. Extend the keyword lists there so new UI stays consistent.

**Product card promo bar** — Salesforce `CTA_Label__c` / `CTA_Color__c` / `CTA_Color_Hover__c` on `vaProductgroup__c` are mirrored as `badge`, `cta_color`, and `cta_color_hover` on `GET /store/events`. `PlpEventCard` renders them via `ProductCardCtaBar` (full-width bar under the card; hover uses the hover color). Homepage column product cards use the same bar when Sanity mirror fields `ctaColor` / `ctaColorHover` are set; otherwise the block falls back to keyword-based badge styling. Those column cards keep an `88px`-wide thumbnail that stretches to the full card height (`object-cover`) so a wrapped title still lines up with the image. Thumbnails do not scale on hover — zooming the `fill` image in that narrow strip shifted the crop sideways.
...
**Unavailable catalog:** `commerceClient.getEvent` returns `null` only on Medusa **404**. When Medusa is down or returns another error, the call throws, so `/ons-aanbod/[handle]` and `/va-thuis/[handle]` render the branded **500** page (`ErrorView`) instead of the activity-not-found page. Missing-page views (`NotFoundView` and the PDP not-found) reset window scroll on mount so a client navigation from a long listing or PDP still shows the 404 heading.

**Header badges** (`PdpHeader`): category badges link to `/ons-aanbod/{category}`. Reis / studiedag / wandeling / rondleiding / workshop use the Salesforce `product_type` chip and link to `/ons-aanbod/{type}`. Other products keep the EventGroup `record_type` chip (`collegereeks`, `lezing`, …) linking to `/ons-aanbod?record_type=…`. VA Thuis record types link to `/va-thuis`.

**Image gallery** (`PdpImageGallery`): product images render as a compact row of thumbnails (up to four, deduped from `gallery_images` / `image_urls`). The first tile uses `priority` and `fetchPriority="high"` for LCP; later tiles are lazy-loaded. **Mobile:** left-aligned horizontal carousel (~**60%** slide width; native snap swipe; a trailing spacer lets the last slide snap flush left). Tap a peeking slide to open it. **Chevron** prev/next on white circular controls flanking **dot** indicators; **from `sm`:** **1×4** row. Tiles use **3:2** aspect ratio. Per-image **captions** come from Salesforce `Image_N_Source__c` (Afbeelding N Tekst) via `gallery_images[].caption`. On **desktop**, the caption appears as a white overlay **below** the tile on hover. On **mobile**, tap the **active** image to toggle the caption under it. Composite credits that use ` | ` in Salesforce are shown as two lines. Salesforce often stores HTML entities (`&eacute;`, `&#39;`, `&nbsp;`) and occasionally Latin-1 mojibake; both are decoded before display so overlay and alt show the real characters. Inline `<em>` / `<strong>` in captions render as emphasis in the hover overlay; other HTML is stripped. Long credits are shown in full (no line clamp).

**Highlighted teacher** (`PdpFeaturedInstructor` in `PdpBookingPanel`): when Salesforce `Highlighted_Teacher__c` is set and the product-docent link exists, the booking panel shows photo, name, title, and bio (`Web_Body__c` as plain text). The same docent can be highlighted on several product groups. Same block on regular and VA Thuis PDPs (dark variant on VA Thuis).

**Sessions** (`PdpLocationTabs`): hybrid products show a delivery filter (**Beide** | **Online** with camera icon | **Fysiek** with pin icon). Online and offline sessions share one list; each row shows a blue camera (online) or red pin (offline) before the location. **Beide** lists online and offline sessions together; a city tab row (**Alle locaties** + per city) appears for **Beide** and **Fysiek**, not for **Online** (per-city tabs filter offline sessions only). With multiple sessions, a sort dropdown (datum / locatie) appears beside the **Data en locaties** heading and **Locatie** / **Datum** column headers are clickable (toggle asc/desc). On **mobile** (`< md`), delivery and city tab rows keep left alignment with the page container but **bleed to the right viewport edge** (tabs cut off); **`max-md:pr-4`** adds trailing space after the last tab when scrolled to the end. Horizontal scroll uses the **`scrollbar-va`** utility (dark gray thumb with subtle yellow accent on a light gray track). Each session is a two-column card: the left column is city (with the delivery-icon indent), venue, date, time, then instructor; the right column is price, availability, and **Direct inschrijven**; from **`md` up**, column tables are used (venue as second line under city when set). Instructor names are **hoverable** when a profile (photo and/or bio) is available: a card shows image, name, and description (`PdpInstructorHoverCard`). On touch, tap the name to toggle the card.

**Session availability** (`sessionTableAvailabilityPresentation`): **Wachtlijst** at 0 (session CTA opens **`WaitlistModal`** with `variant_id`); **Nog N plaats(en)** when `available_quantity ≤ lowStockThreshold` (Sanity default 5); **Beschikbaar** when **10+** spots remain; otherwise **N beschikbaar** (e.g. 6–9 spots). Session CTAs share a **14rem** min-width so **Wachtlijst** lines up with **Direct inschrijven** (mobile session cards use **11rem** so the date, time, and teacher fit beside the button). Sold-out CTAs use `#bfbfbf` / hover `#4c4c4c`; almost-full sessions (remaining ≤ 30% of `event_item.capacity`, rounded up; ≤ 3 when capacity unknown) show **Bijna vol** in `#f7373d` / hover `#E0282E` instead of **Direct inschrijven**. **Product-level sold out** (`eventIsFullySoldOut` in PLP cards, JSON-LD): only when **every** bookable session is sold out — not when `min_available_quantity` is 0 because a single date sold out. Sold-out CTAs on the booking panel, session table, agenda rows, and fully sold-out PLP cards open **`WaitlistModal`** → `POST /store/events/:handle/waitlist` (optional `variant_id` for a single session; phone required, 8–15 digits). The booking panel orange signal also covers low stock and “Inschrijving sluit bijna”; it does not show a start-soon countdown.
...
6. **Homepage / category tiles / header-footer out of date on staging** – CMS pages (`app/(main)/page.tsx`, `app/(main)/[...slug]/page.tsx`) and the main layout header/footer use `revalidate = 60`. Publishing in Sanity should bust the cache immediately via `POST /api/revalidate/sanity` (see `docs/DEPLOYMENT.md` § Performance & caching). Include `_type in ["page", "generalSettings", "menu", "category"]` on the webhook (projection `slug` via `coalesce(slug.current, slug)` so category string slugs arrive). **Footer or menu changes** revalidate the `general-settings` Data Cache, both layout trees, then queue batched `revalidatePath` for all storefront URLs (including private chrome). **Category** publishes bust `/`, `/va-thuis`, and `/ons-aanbod/{slug}` — tile images live on the category document and stay hidden until that document is published. **Missing primary menu** usually means `generalSettings` is draft-only — publish it in Studio. Without the webhook, wait up to 60 s or redeploy. For a single CMS page, Studio **document ⋯ → Clear page cache** calls `POST /api/revalidate/studio`.
