...
**Plaats filter** (`PlpFilterSidebar`): without a search query, the city checklist shows **5** options, then a white **Meer bekijken** row with a down chevron. After expanding, all places are listed; if there are **more than 10**, the list scrolls inside a max-height region (~10 rows). Typing in the plaats search shows matching results in the same scroll pattern when needed.
