...
**Hero block**: Multi-slide slider shows **yellow chevron** previous/next controls at the **bottom left and right**, with **dot** indicators centered along the bottom. Arrows sit above slide links so they change slides without navigating. Autoplay follows Sanity **Autoplay** / **Autoplay Interval**. The right-column **Meld je aan** newsletter card follows Sanity **Show newsletter** (`newsletterEnabled`; unset or true = shown, false = hidden; top panel then fills the column). **LCP**: the first slide background is rendered via `SanityImage` with `priority` and `fetchPriority="high"` (no `loading="lazy"`), passed from `BlockRenderer` as a server slot so the image is discoverable in the initial HTML; carousel slides after the first stay lazy-loaded. Other CMS images (`SanityImage` without `priority`) remain lazy by default. `SanityImage` in **`fill`** / **`contain`** mode requests `w` + `fit=max` (no forced 16:9 crop) so CSS `object-fit` can use the source aspect; default quality is **80**. Next 16 allowlists image qualities in `next.config.js` (`[75, 80, 90]`).
