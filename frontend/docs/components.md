...
**Sessies** (`PdpLocationTabs`): on **mobile** (`< md`), one card per session with fields stacked vertically (city, optional venue from `event_item.location_name` / Salesforce `Product_Location_Name__c`, date, time, docent from `event_item.instructor_name`, price, availability, full-width CTA). From **`md` up**, the original column table is shown (venue as second line under city when set). City tabs unchanged when multiple locations exist.
