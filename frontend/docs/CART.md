...
- Toegepaste cadeaubonnen worden opnieuw afgezet tegen het actuele cart-totaal via `syncGiftCardCredits` (`syncAppliedGiftCardCredits` in `cart.ts`): bij laden van de winkelwagen, na **add-to-cart**, na wijziging van aantallen, en na het toepenen van een kortings-/boncode. Header, winkelwagen en checkout roepen dat samen aan. De storefront deelt één request; Medusa neemt daarnaast een database-lock per cart, omdat losse workers elkaar anders niet zien en de cadeaubon twee keer boeken.
...
| `CartItemRow` | Thumbnail (4:3, same crop as product tiles), title, `CartLineItemDetails`, qty selector (locked at **1** without −/+ for cadeaubon purchases and VA Thuis bundles), remove |
