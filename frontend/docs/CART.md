...
- Toegepaste cadeaubonnen worden opnieuw afgezet tegen het actuele cart-totaal via `syncGiftCardCredits` (`syncAppliedGiftCardCredits` in `cart.ts`): bij laden van de winkelwagen, na **add-to-cart**, na wijziging van aantallen, en na het toepenen van een kortings-/boncode.
