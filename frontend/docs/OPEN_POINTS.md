...
- **Ons aanbod / Agenda**: inline `PlpSearchBar` + `?q=` filtered via `GET /store/events` / `GET /store/agenda` (all query terms must match title/onderwerp/docent/plaats/locatie; light title typos when OpenSearch is configured).
