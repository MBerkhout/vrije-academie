...
- `formatPriceEur(cents, 'whole' | 'standard')` — `standard` (default) shows cents on PLP cards and PDP session tables; whole euro amounts use Dutch “,-” instead of “,00”; `whole` omits decimals (booking panel headline); cart/checkout use `standard`.
...
**PDP sessions**: `GET /store/events/:handle` omits variants whose `event_item.start_at` is in the past; the commerce client applies the same filter as a safeguard. When an event has sessions in more than one city, `PdpLocationTabs` shows an **Alle locaties** tab first (default selected) with every session sorted by date; per-city tabs filter the list. On mobile (`< md`), each session is a vertical stack; from `md` up the session table is used. Session **Beschikbaarheid**: uitverkocht → “Volgeboekt”; ≤ `lowStockThreshold` (default 5) → “Nog N plaats(en)”; **10+ vrije plaatsen** → “Beschikbaar”; otherwise → “N beschikbaar”. Logic in `sessionTableAvailabilityPresentation`. **Reizen with external registration**: when Medusa returns `external_registration_url` (Salesforce `External_Registration_URL__c`), **Direct inschrijven** in `PdpBookingPanel` and session rows opens that URL in a new tab instead of the cart. **VAthuis** products (`purchase_mode: bundle_only`) show `PdpEpisodesTable` with a chapter selector, episode rows (aflevering, duur, beschrijving), **Bekijk aflevering** preview modal (`PdpEpisodePreviewModal` + Audience Player iframe), and **Koop alle lessen** on locked rows (scrolls to `#booking-panel`). Copy for the episodes table and bundle CTA is editable in **General settings → PDP → UI labels** (`episodesHeading`, `chapterLabel`, column headers, `watchEpisode`, `bundleCta`). Session table column **Docent** (not “Instructeur”). Category badges in `PdpHeader` link to `/ons-aanbod/{category-slug}` when a slug is present.
