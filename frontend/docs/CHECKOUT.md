...
**Gegevens aanpassen (edit mode):** From step 3, **Gegevens aanpassen** and the stepper link back to step 2 use `/checkout/inloggen?bewerken=1`. With that query param, a logged-in customer with a complete profile stays on the details form (prefilled from the account) instead of being auto-skipped to betaling; a **guest** with complete cart shipping sees the same details form prefilled from the **cart** (e-mail included). Both get **Terug naar betaling**. They can save and continue to betaling, go back without saving, or (logged-in only) **Uitloggen** to switch account or continue as guest. **Wijzigen** next to the order-confirmation e-mail on betaling uses `/checkout/inloggen?wijzig-email=1` instead: the e-mail step stays open (no auto-skip). Submitting the current address returns to betaling; a different address logs out first, then follows lookup → login or guest details.
...
**`CheckoutPaymentOrderOverview`** (above the form): loads cart + **`fetchCartExtras`**. All line items via **`OrderSummaryLineItems`** (bold title, 4:3 thumbnail matching the cart and product tiles, **`buildLineItemQuantityLabel`**, **`onlineCityFallback`**) + **`OrderSummaryTotalsBlock`**. Same component in the sidebar on inloggen/bevestiging. Cadeaubon purchase lines use **`/branding/cadeaubon-thumb.jpg`** (`resolveLineItemThumbnail`). Totals use gross producten + **waarvan BTW (X%)** (see `docs/CART.md` → Pricing & BTW).

**`CheckoutPaymentForm`** loads cart via `getActiveCart()` (includes **`syncAppliedGiftCardCredits`** when `metadata.gift_card_redemptions` is set, so cadeaubon credit lines match the current total). When the cart total is positive, loads `GET /store/payment-providers?region_id={cart.region_id}` (effect re-runs if the total crosses zero, e.g. gift card removed). Renders:
...
4. Trust signals
5. Primary CTA — `Betaal € {totaal}` when an amount is due; `Bestelling plaatsen` when the total is €0. While the redirect is in flight the label is `Je wordt doorgestuurd…` (paid) or `Bestelling plaatsen…` (€0) and the button stays disabled, including after a successful Mollie or thank-you navigation, so it cannot be clicked twice. Directly above the button: “Je bestelbevestiging sturen we naar {e-mail}. Wijzigen” — **Wijzigen** goes to `/checkout/inloggen?wijzig-email=1`, which opens the e-mail step (prefilled). The same address returns to betaling; a different address logs the current customer out and continues the normal lookup. Above that line: the NL notice that clicking pay accepts the terms; `voorwaarden` links to `/algemene-voorwaarden`. Under the button, when an amount is due: “🔒 Je betaalt veilig via deze link. Daarna kom je automatisch terug bij Vrije Academie.”
