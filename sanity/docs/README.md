...
- **Page**: Main page documents with an array of content blocks. The PLP route (`/ons-aanbod`) is edited as Page `pageOnsAanbod` with a [PLP block](./PLP.md) (`plpBlock`). The gift card purchase page uses Page `pageCadeaubon` with a [Cadeaubon block](./CADEAUBON.md) (`giftCardBlock`); URL = Page slug. **VA Thuis** pages use the same `page` type with **VA Thuis page** enabled (`isVaThuis`); slug must start with `va-thuis` (landing: `va-thuis`, sub-pages: `va-thuis/…`). Studio: **Pages → VA pages** / **Pages → VA Thuis pages**. Each of those panes has a search field at the top that matches title or slug, including pages inside folders. Lists are grouped recursively by slug path (e.g. `/over-ons/team` appears under an `over-ons` folder); open **This page** to edit the folder’s own page, or use **+** inside a folder to create a nested page with the slug prefix prefilled. Draft and published versions of the same page appear once (the draft slug/title if they differ). Pages without a slug yet can’t be placed in the tree, so they show under a "Missing slug" section (⚠) at the list root instead. Landing blocks: `vathuisHeroBlock`, `vathuisCategoriesBlock`, `vathuisProductRowBlock`, `vathuisTeachersBlock`, `vathuisPromoTilesBlock` (plus generic blocks). Migrate legacy `vathuisBlock`: `npm run migrate:vathuis-landing-to-blocks`. The document ⋯ menu has **Clear page cache** to bust storefront ISR for that slug (`POST /api/revalidate/studio`).
...
# Same value as frontend SANITY_STUDIO_REVALIDATE_SECRET (page ⋯ → Clear page cache)
