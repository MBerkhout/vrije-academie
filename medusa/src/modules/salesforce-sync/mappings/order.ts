...
function formatSfDate(iso: string | Date | undefined): string {
...
export function orderHeaderToSalesforce(o: MedusaOrderPushShape): Partial<SfOrderShape> {
  ...
  const description = useMedusaFields
    ? o.currency_code
      ? `currency:${o.currency_code}`
      : undefined
    : [
        `medusa:${o.id}`,
        o.display_id != null ? `#${o.display_id}` : null,
        o.email ?? null,
      ]
        .filter(Boolean)
        .join(" ")
  ...
  return {
    ...
    Payment_Method__c:
      o.payment_method && o.payment_method !== "GRATIS" ? o.payment_method : undefined,
