...
export async function GET(req: AuthenticatedMedusaRequest, res: MedusaResponse): Promise<void> {
  ...
  const base = await resolveAdminSalesforceInstanceBase(req.scope)
  const mapped = items.map((row) => {
    const entityType = row.entity_type as "customer" | "order" | "product" | "variant"
    const accountId = row.salesforce_account_id?.trim() || null
    const contactId = row.salesforce_id?.trim() || null
    const sfObject =
      entityType === "customer" && accountId
        ? "Account"
        : salesforceObjectForEntity(entityType)
    const sfId = entityType === "customer" && accountId ? accountId : contactId
    ...
    return {
      ...
      openInSalesforceUrl:
        sfId && base ? salesforceRecordViewUrl(base, sfObject, sfId) : null,
    }
  })
