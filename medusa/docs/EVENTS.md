...
- `GET /store/events/:handle` — single product by handle with full enrichment: `tags`, `variants.prices`, `images`, `categories`, `instructors`, computed `has_free_trial`, `image_urls`, `price_from`, `min_available_quantity`, `has_linked_online_sessions` (true when `metadata.salesforce_linked_online_productgroup_id` is set — linked Zoom catalog merged onto this product), etc. **`variants`** only include rows whose linked `event_item` has no `start_at` or `start_at >= now` (past sessions are omitted; non-event variants such as VAthuis bundles are kept). Group aggregates (`cities`, `earliest_start_at`, `min_available_quantity`, …) are derived from that filtered set. **Prices** in JSON responses are integer **cents** (Medusa stores major EUR units internally; converted in `src/lib/medusa-price-to-cents.ts`).
