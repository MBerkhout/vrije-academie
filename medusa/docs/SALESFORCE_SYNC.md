...
| Gift card purchase | `OrderItem` + `Voucher__c` | `Giftcard_*` fields; voucher sync state `entity_type: voucher`. Medusa schrijft **geen** Medusa-hex naar `Code__c`. Na upsert: GTC uit `Voucher__c.Code__c` of `Name` → `gift_card.code` + ontvanger-mail. |
| Gift card redemption | voucher `OrderItem` | negative amount, `Voucher__c` lookup; `Discount_Code__c` = opgeslagen code (`GTC-…` of `GIFT-…`). Lookup fallback: `Code__c` of `Name`. |
| Gift card redeem (store) | Medusa `gift_card` | Onbekende code → lazy import uit `Voucher__c` ([`explore-gift-card-from-salesforce.ts`](../src/lib/explore-gift-card-from-salesforce.ts)); geen bulk sync van alle vouchers. |
...
**Pricing:** Salesforce gross price is source of truth (`Price__c` on each `vaProduct__c`, e.g. `26.5` is already incl. VAT; `Net_Price__c` is the formula `Price__c / (1 + VAT_Rate__r.VAT_Percentage__c)` and is not imported). Medusa stores that gross amount. Prices are **tax-inclusive**; run `npm run seed:region` so Medusa extracts VAT instead of adding it on top. Cart refetches that compute `total` (gift cards, checkout) must select `items.is_tax_inclusive`. Omitting it makes Medusa treat the gross price as net and add the product-type rate (for example BTW laag 9%) on top; that inflated amount is what gets pushed as the Salesforce voucher `UnitPrice`. Order product lines still send `Price__c` as `UnitPrice`; Salesforce then derives `OrderItem.Total_Net_Price__c` as `UnitPrice / (1 + VAT_Percentage__c)`.
...
- **`EventGroup.record_type`** → `vathuis` (excluded from Ons aanbod / Agenda; listed via **`GET /store/vathuis`**; included in `/zoeken`). Import busts the listing cache then reindexes OpenSearch from the database so a stale snapshot cannot drop the bundle from search.
